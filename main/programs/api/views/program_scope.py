...
from ..serializers import ProgramScopeSerializer, ProgramScopeListSerializer
from ...models import ProgramScope


class ProgramScopeView(ModelViewSet, APIResponseBase):
    permission_classes = [IsAuthenticated]
    serializer_class = ProgramScopeSerializer
    lookup_field = "guid"

    def get_queryset(self):
        return ProgramScope.objects.filter(program__slug=self.kwargs["program_slug"])

    @swagger_auto_schema(query_serializer=ProgramScopeListSerializer, responses={status.HTTP_200_OK: ProgramScopeSerializer})
    def list(self, request, program_slug=None, guid=None, *args, **kwargs):
